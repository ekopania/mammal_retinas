...
#For data with high certainty only, include eco_certain, eco_certain2, ret_certain
eco_keep<-Reduce(intersect, list(eco_noMissing, eco_noMissing2))
...
write.nexus(tree_keep, file=paste0(tree,"_acAny_",eco_cat,".",eco_trait,"_",eco_cat2,".MandAr.nexus"))
write.table(newdata, file=paste0("acAny_",eco_cat,".",eco_trait,"_",eco_cat2,".MandAr_",tree,".noMissingData.txt"), col.names=FALSE, row.names=TRUE, quote=FALSE, sep="\t")
...
#clean up retinal structure names to have no spaces
#End with ".certainOnly.nexus" if filtering on data certainty
write.nexus(tree_keep, file=paste0(tree,"_acOnly_",eco_cat,".",eco_trait,"_",eco_cat2,".MandAr.nexus"))
write.table(newdata, file=paste0("acOnly_",eco_cat,".",eco_trait,"_",eco_cat2,".MandAr_",tree,".noMissingData.txt"), col.names=FALSE, row.names=TRUE, quote=FALSE, sep="\t")
