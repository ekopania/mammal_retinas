...
for(ret in rets){
	...
	write.table(newdata, file=paste0(retname,"Any_",eco_cat,"_",tree,".noMissingData.threshold.txt"), col.names=FALSE, row.names=TRUE, quote=FALSE, sep="\t")
	...
	print(paste("Getting data for", ret, "(only)"))
        ...
        #This will overwrite several times, but it's okay because should be the same tree for all combinations of same eco_cat
        write.nexus(tree_keep, file=paste0(tree,"_retinalStructure_",eco_cat,".threshold.nexus"))
        write.table(newdata, file=paste0(retname,"Only_",eco_cat,"_",tree,".noMissingData.threshold.txt"), col.names=FALSE, row.names=TRUE, quote=FALSE, sep="\t")
}
