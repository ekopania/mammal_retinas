...
keep<-Reduce(intersect, list(retina_noMissing, eco_noMissing_sp, eco_certain_sp, mytree$tip.label)) #ret_certain
...
write.nexus(tree_keep, file=paste0(tree,"_acAny_",eco_cat,".",eco_trait,".threshold.nexus"))
write.table(newdata, file=paste0("acAny_",eco_cat,".",eco_trait,"_",tree,".noMissingData.threshold.txt"), col.names=FALSE, row.names=TRUE, quote=FALSE, sep="\t")
...
rownames(newdata)<-retina_keep$Hub_Species
#clean up retinal structure names to have no spaces
write.nexus(tree_keep, file=paste0(tree,"_acOnly_",eco_cat,".",eco_trait,".threshold.nexus"))
write.table(newdata, file=paste0("acOnly_",eco_cat,".",eco_trait,"_",tree,".noMissingData.threshold.txt"), col.names=FALSE, row.names=TRUE, quote=FALSE, sep="\t")
