...
for(ret in rets){
	for(eco_trait in eco_traits){
		...
		write.table(newdata, file=paste0(retname,"Any_",eco_trait,"_",tree,".noMissingData.certainOnly.terrestrial.txt"), col.names=FALSE, row.names=TRUE, quote=FALSE, sep="\t")
		...
		print(paste("Getting data for", ret, "(only)"))
                ...
                #This will overwrite several times, but it's okay because should be the same tree for all combinations of same eco_cat
                write.nexus(tree_keep, file=paste0(tree,"_retinalStructure_",eco_cat,".certainOnly.terrestrial.nexus"))
                write.table(newdata, file=paste0(retname,"Only_",eco_trait,"_",tree,".noMissingData.certainOnly.terrestrial.txt"), col.names=FALSE, row.names=TRUE, quote=FALSE, sep="\t")
	}
}
