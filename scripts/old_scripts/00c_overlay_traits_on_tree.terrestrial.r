#PURPOSE: Overlay retinal specialization and ecological traits on zoonomia tree - terrestrial species only! (no aquatic or fossorial)
...
mydata0<-read.csv("mam241.pheno.terrestrial.csv")
...
nomissing<-just.retinal[rowSums(just.retinal=="") < 2,] #3,]
ret_certain<-rownames(just.retinal)[which(just.retinal$certainty=="A")]
keep<-Reduce(intersect, list(inall, rownames(nomissing), ret_certain)) #ret_certain
...
sizeBin<-unlist(sapply(elton.sort$BodyMass.Value, function(x) if(x < 100000){1} else{0}))
...
#Plot
pdf("mammal_eyes_tree.terrestrial.withEltonPheno.certainOnly.pdf", width=17, height=22, useDingbats=FALSE, onefile=TRUE)
...
pdf("mammal_eyes_tree.terrestrial.hz_grazer.certainOnly.pdf", width=17, height=22, useDingbats=FALSE, onefile=TRUE)
...
pdf("mammal_eyes_tree.terrestrial.ac_predator.certainOnly.pdf", width=17, height=22, useDingbats=FALSE, onefile=TRUE)
